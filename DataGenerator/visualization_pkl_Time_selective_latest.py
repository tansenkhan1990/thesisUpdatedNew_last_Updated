...
#for Dbpedia
df_1 = fifthopole_df.loc[(fifthopole_df['time']>=1900) & (fifthopole_df['time']<=1920)].reset_index(drop=True)
df_2 = fifthopole_df.loc[(fifthopole_df['time']>=1921) & (fifthopole_df['time']<=1940)].reset_index(drop=True)
# df_3 = fifthopole_df.loc[(fifthopole_df['time']>=1931) & (fifthopole_df['time']<=1940)].reset_index(drop=True)
df_4 = fifthopole_df.loc[(fifthopole_df['time']>=1941) & (fifthopole_df['time']<=1960)].reset_index(drop=True)
# df_5= fifthopole_df.loc[(fifthopole_df['time']>=1951) & (fifthopole_df['time']<=1960)].reset_index(drop=True)
df_6 = fifthopole_df.loc[(fifthopole_df['time']>=1961) & (fifthopole_df['time']<=1980)].reset_index(drop=True)
# df_7 = fifthopole_df.loc[(fifthopole_df['time']>=1971) & (fifthopole_df['time']<=1980)].reset_index(drop=True)
df_8 = fifthopole_df.loc[(fifthopole_df['time']>=1981) & (fifthopole_df['time']<=2000)].reset_index(drop=True)
# df_9 = fifthopole_df.loc[(fifthopole_df['time']>=1991) & (fifthopole_df['time']<=2000)].reset_index(drop=True)
df_10 = fifthopole_df.loc[(fifthopole_df['time']>=2001) & (fifthopole_df['time']<=2020)].reset_index(drop=True)
# df_11 = fifthopole_df.loc[(fifthopole_df['time']>=2001) & (fifthopole_df['time']<=2010)].reset_index(drop=True)
#df_10 = fifthopole_df.loc[(fifthopole_df['time']>=1870) & (fifthopole_df['time']<=1880)].reset_index(drop=True)

#
time_dfs = [df_1,df_2, df_4,df_6,df_8,df_10]
# time_dfs = [df_1,df_2,df_3,df_4]
times_in_time_category = pd.DataFrame()
for df in time_dfs:
    range_start = df['time'].min()
    range_end = df['time'].max()
    times_within_time_range = list(df.loc[(df['time']>=range_start) & (df['time']<=range_end)]['time'].unique())
    ...
    times_in_time_category = times_in_time_category.append(pd.DataFrame(np.array([time_category, times_within_time_range])).T)

times_in_time_category = times_in_time_category.reset_index(drop=True)

times_in_time_category.columns = ['category', 'matched_entities']
...
#for wiki
times_in_time_category.to_pickle('/home/tansen/my_files/thesis_new_files/thesisUpdatedNew/dataset/Wikidata5/time_category_wise_times.pkl')
